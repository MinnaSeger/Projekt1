...
public class MenyProjektHandlaggare extends javax.swing.JFrame {
    ...
    public MenyProjektHandlaggare(InfDB idb, String dbAid) {
        ...
        fyllKoppladeProjektTabell();
        fyllAvdelningsProjektTabell();
        fyllStatusComboBox();
    }

    // 1. Fyll tabell med tilldelade projekt (projektchef)
    ...
    // 2. Fyll tabell med projekt kopplade till anställd (via ans_proj)
    private void fyllKoppladeProjektTabell() {
        try {
            ...
            ArrayList<HashMap<String, String>> resultat = idb.fetchRows(query);
            DefaultTableModel model = (DefaultTableModel) tblAvdelningsProjekt.getModel();
            model.setRowCount(0);
            ...
            JOptionPane.showMessageDialog(this, "Fel vid hämtning av kopplade projekt: " + e.getMessage());
        }
}
    

    private void fyllAvdelningsProjektTabell() {
    try {
        String query = "SELECT projekt.projektnamn, projekt.startdatum, projekt.slutdatum, projekt.status " +
                       "FROM projekt " +
                       "JOIN ans_proj ON projekt.pid = ans_proj.pid " +
        ...
        if (resultat != null) {
            for (HashMap<String, String> rad : resultat) {
                model.addRow(new Object[]{
                    rad.get("projektnamn"), rad.get("startdatum"), rad.get("slutdatum"), rad.get("status")
                });
            }
        }
    } catch (InfException e) {
    ...
    // 4. Fyll status-väljaren med unika statusar
    ...
    // 5. Filtrera avdelningens projekt efter vald status
    private void filtreraAvdelningsProjekt() {
